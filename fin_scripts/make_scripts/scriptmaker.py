...
for input_file in input_file_arr :
	name=input_file[:-3]
	script_file=name+"sh"
	output_path=output_path+name[:-1]+"/"
	...
	exec_string="""mpiexec	--oversubscribe -np 4  python3  main.py -p $INPUTDIR -p2 $OUTPUTDIR -name ${INPUT_FILES[0]} -num ${SLURM_ARRAY_TASK_ID} -procs 4 -index ${atom_index} 
	"""
